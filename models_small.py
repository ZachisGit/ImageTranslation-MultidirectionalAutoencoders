...
enc_channels = [64,64,16]
dec_channels = [64,128,64]
...
### DECODERS ###
def decX(L,out_channels,summary=True):
	global dec_x_reuse
	...
	with tf.variable_scope('decX',reuse=dec_x_reuse):
		...
		for i in range(len(kernel_sizes)):
			c_w *= sample_rates[i]
			c_h *= sample_rates[i]
			model = upsample(model,dec_channels[i],kernel_sizes[i],sample_rates[i],'decX_upsample-ksize_'+str(kernel_sizes[i])+'-wh_'+str(c_w)+'x'+str(c_h),activation=lrelu,summary=summary)
...
def decY(L,out_channels,summary=True):
	global dec_y_reuse
	...
	with tf.variable_scope('decY',reuse=dec_y_reuse):
		...
		for i in range(len(kernel_sizes)):
			c_w *= sample_rates[i]
			c_h *= sample_rates[i]
			model = upsample(model,dec_channels[i],kernel_sizes[i],sample_rates[i],'decY_upsample-ksize_'+str(kernel_sizes[i])+'-wh_'+str(c_w)+'x'+str(c_h),activation=lrelu,summary=summary)
